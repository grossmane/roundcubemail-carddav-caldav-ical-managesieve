...
class calendar extends rcube_plugin
{
  ...
  public function startup($args)
  {
    ...
    if ($args['task'] == 'calendar' && $args['action'] != 'save-pref') {
      ...
      $this->register_action('oauth_handle_callback', array($this, 'oauth_handle_callback'));
      $this->register_action('oauth_logout', array($this, 'oauth_logout'));
      $this->register_action('oauth_status', array($this, 'oauth_status'));
      $this->register_action('oauth_redirect', array($this, 'oauth_redirect'));
  ...
  /**
   * Creates a oauth_client instance from the specified provider which is determined as follows:
   *
   * - Specified as "provider_id" in the current GET request
   * - Encoded in the state specified as "state" oder "_state" in the current GET request
   *
   * @return oauth_client
   */
  private function oauth_create_instance()
  {
    $provider_id = get_input_value("provider_id", rcube_utils::INPUT_GET);
    if(!$provider_id) {

      // Decode state to get provider
      $state = oauth_client::decode_state();

      // Load provider
      if(isset($state["provider_id"]))
        $provider_id = $state["provider_id"];
    }

    $provider = oauth_client::get_provider($provider_id);

    if(!$provider)
      rcube::raise_error("Invalid OAuth provider \"$provider_id\".", true, true);

    return new oauth_client($this->rc, $provider);
  }

  /**
   * Callback for OAuth requests.
   * @see oauth_client::handle_callback()
   */
  public function oauth_handle_callback()
  {
    $oauth = $this->oauth_create_instance();
    $oauth->handle_callback();

    // Pass execution to parent window again ...
  ...
  public function oauth_logout()
  {
    ...
    echo "done"; // TODO: Probably s.th. like this: $this->rc->output->command('plugin.update_counts', array('counts' => $counts));
    exit;
  }

  /**
   * Returns the status of the given oauth provider.
   */
  public function oauth_status()
  {
    $oauth = $this->oauth_create_instance();

    echo $this->rc->output->json_serialize(array(
      "is_logged_in" => $oauth->has_access_token()
    ));
    exit;
  }

  /**
   * Generate an authorize URI and redirect.
   */
  public function oauth_redirect()
  {
    $oauth = $this->oauth_create_instance();
    header("Location: ".$oauth->get_authorize_uri());
    exit;
  }
...
//      $chars = "!# abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ 1234567890";
  ...
  private function write_preprocess(&$event, $action)
  {
    ...
    $event['start'] = new DateTime(substr($event['start'], 0, 33), $this->timezone);
    $event['end'] = new DateTime(substr($event['end'], 0, 33), $this->timezone);
  ...
  public function print_view()
  {
    ...
    if ($driver = rcube_utils::get_input_value('driver', rcube_utils::INPUT_GPC))
      $this->rc->output->set_env('driver', $driver);
    elseif ($driver = rcube_utils::get_input_value('_driver', rcube_utils::INPUT_GPC))
      $this->rc->output->set_env('driver', $driver);
    else {
      $driver = $this->rc->config->get("calendar_driver_default", "kolab"); // Fallback to kolab if nothing was configured.
      $this->rc->output->set_env('driver', $driver);
    }    
