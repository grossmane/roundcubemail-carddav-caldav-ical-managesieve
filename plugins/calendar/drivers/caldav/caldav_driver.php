...
class caldav_driver extends calendar_driver
{
    ...
    private $sync_period = 1; // TODO 0; // seconds
    ...
    protected function _read_calendars()
    {
        ...
        if (!empty($this->rc->user->ID)) {
            ...
            while ($result && ($arr = $this->rc->db->fetch_assoc($result))) {
                ...
                $arr['readonly'] = intval($arr['readonly']);
                $arr['editable'] = true;
                $arr['deletable'] = true;
    ...
    public function list_calendars($filter = 0)
    {
        ...
        foreach ($calendars as $idx => $cal) {
            ...
            if ($filter & self::FILTER_ACTIVE && !$cal['active']) {
                unset($calendars[$idx]);
            }

            // filter read-only calendars
    ...
    public function create_calendar($cal)
    {
        ...
        $calendars = $this->_autodiscover_calendars($cal);
        $cal_ids = array();
        ...
        if($calendars)
        {
            ...
            foreach ($calendars as $calendar)
            {
	            ...
	            $result = $this->rc->db->query("SELECT * FROM ".$this->db_calendars." WHERE caldav_url LIKE ? AND user_id = ?", $calendar['href'], $this->rc->user->ID);
    ...
    public function edit_calendar($cal)
    {
        $query = $this->rc->db->query("UPDATE " . $this->db_calendars . "
            SET   name=?, color=?, showalarms=?, readonly=?, caldav_url=?, caldav_tag=?, caldav_user=?, caldav_oauth_provider=?
            WHERE calendar_id=?
            AND   user_id=?",
            $cal['name'],
            $cal['color'],
     ...
     * @param bool Force read-only calendars to create this event e.g. when syncing remote events.
     * @see calendar_driver::new_event()
     * @return bool|mixed
     */
    public function new_event($event, $force_readonly=false)
    {
        ...
        if (!empty($this->calendars)) {
            ...
            $cal = $this->calendars[$event["calendar"]];
            if($force_readonly || !$cal["readonly"]) {
                ...
                if ($event = $this->_save_preprocess($event)) {
                    ...
                    if (!isset($event["caldav_tag"])) {
                        $event = $sync_client->create_event($event);
                        if (!$event)
        ...
        if(!$cal["readonly"]) {
            ...
            if ($this->_db_edit_event($event)) {
                ...
                if ($success === true) {
                    ...
                    // Undo changes in events, return null (not true) to suppress further dialogs
                    if ($this->_db_edit_event($old_event))
                        return null;
                }
            }
        }
    ...
    public function remove_event($event, $force = true)
    {
        ...
        return null; // Don't show error since it is already handled
    }
    ...
    private function _read_postprocess($event)
    {
        ...
        $cal = $this->calendars[$event['calendar_id']];
        $event['editable'] = !$cal['readonly'];
    ...
    public function calendar_form($action, $calendar, $formfields)
    {
        ...
        // Get oauth2 providers with valid access token
        $oauth_providers_table = new html_table(array("class" => "propform"));
        foreach (oauth_client::get_providers() as $id => $provider ) {

            $oauth_providers_table->add(array("class" => "title"), $provider["name"]);
            $oauth_client = new oauth_client($this->rc, $provider);

            $oauth_providers_table->add(null,
                html::tag("input", array(
                    "type" => "radio",
                    "name" => "caldav_oauth_provider",
                    "value" => $id,
                    "checked" => ($calendar["caldav_oauth_provider"] == $provider["id"]) ? "checked" : null)));

            $oauth_providers_table->add(null,
                $this->rc->output->button(array(
                    'prop' => json_encode(array(
                        'provider_id' => $provider["id"],
                        'calendar' => isset($calendar["id"]) ? $calendar["id"] : null)),
                    'command' => 'oauth-logout',
                    'label' => 'calendar.logout',
                    'class' => 'oauth-logout',
                    'style' => !$oauth_client->has_access_token() ? "display: none": "")).

                $this->rc->output->button(array(
                    'prop' => json_encode(array(
                        'provider_id' => $provider["id"],
                        'authorize_uri' => $oauth_client->get_authorize_uri(),
                        'calendar' => isset($calendar["id"]) ? $calendar["id"] : null)),
                    'command' => 'oauth-login',
                    'label' => 'calendar.login',
                    'class' => 'oauth-login',
                    'style' => $oauth_client->has_access_token() ? "display: none": "")));

            $oauth_providers_table->add_row();
        }

        $form['auth']['fieldsets']['caldav_auth_oauth'] = array(
            'name' => $this->cal->gettext('caldav_auth_oauth'),
            'content' => $oauth_providers_table->show()
        );
     ...
     *   caldav_user: Username
     *   caldav_pass: Password
     *   caldav_oauth_provider: Instead of user/pass one can also pass the id of an authenticated Oauth2 provider
    ...
    private function _autodiscover_calendars($props)
    {
        ...
        $caldav = new caldav_client($props["caldav_url"], $props["caldav_user"], $props["caldav_pass"], $oauth_client);
    ...
    private function _perform_updates($updates)
    {
        ...
        foreach($updates as $update)
        {
            ...
            if(isset($update["local_event"]))
            {
		        ...
		        $event["attendees"] = $update["local_event"]["attendees"] + $update["remote_event"]["attendees"];
            ...
            else
            {
                ...
                $event_id = $this->new_event($event, true);
                if($event_id)
                {
